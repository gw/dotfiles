...
#+BEGIN_SRC emacs-lisp
  ...
  (defun gw/insert-wikilink ()
    "Select a note and insert it, creating if it doesn't exist."
    (interactive)
    (insert
     (format "[[%s]]"
                               ...
                               (delete-dups
                                (split-string
                                 (shell-command-to-string
                                  "fd . ~/icloud_drive/Documents")
                                 "\n"))))))
    ;; :action isn't working, for some reason, so I just wrapped it all in 'insert
    ;; :action (lambda (tag)
              ;; (insert tag))
    )
  ...
  (defun gw/insert-tag ()
    ...
    (interactive)
    (insert (ivy-read "Select tag: "
              (delete-dups
               (split-string
                (shell-command-to-string
                 "rg -o --no-heading --no-filename --no-line-number \"#\\w+\" ~/icloud_drive/Documents")))))
    ;; :action isn't working, for some reason, so I just wrapped it all in 'insert
    ;; :action (lambda (tag)
              ;; (insert tag))
    )

  (global-set-key (kbd "C-c n") 'gw/insert-wikilink)
  (global-set-key (kbd "C-c t") 'gw/insert-tag)
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun gw/writing-mode()
    (interactive)
    (setq buffer-face-mode-face '(:family "iA Writer Quattro V" :height 160))
    (buffer-face-mode)
    (olivetti-mode)
