...
#+BEGIN_SRC emacs-lisp
  (use-package markdown-mode
    ...
    ;; Open markdown mode when working in zettel-dir.
    ;; Doing it manually here b/c :mode macro doesn't eval
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun gw/all-tags ()
    ...
    (delete-dups
     (split-string
      (shell-command-to-string
       (format "rg -o --no-heading --no-filename --no-line-number \"#\\w+\" %s" gw/zettel-dir)))))
  ...
  (defun gw/munged-filename ()
    ...
    (string-join
     (let ((case-fold-search nil))
       (gw/regexp-list "[A-Z][a-z]*" (file-name-base buffer-file-name)))
     " "))
  ...
  (define-minor-mode gw/zettel-mode
    ...
    :keymap (let ((map (make-sparse-keymap)))
              (define-key map (kbd "C-c n") 'gw/insert-wikilink)
              (define-key map (kbd "C-c t") 'gw/insert-tag)
              map))

  ;; Insert boilerplate when opening new files in zettel-mode
