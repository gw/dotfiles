...
** Change "C-a" go to toggle b/w BOL and first-non-whitespace char
#+BEGIN_SRC emacs-lisp
  (defun gw/smart-move-beginning-of-line ()
    "Move point to first non-whitespace char--if there, move to BOL."
    (interactive)
    (let ((pt (point)))
  ...
  (global-set-key (kbd "C-a") 'gw/smart-move-beginning-of-line)
#+END_SRC
