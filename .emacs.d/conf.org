...
#+BEGIN_SRC emacs-lisp
  ;;; Backlink generation
  ;;; Commentary:
  ;; 1. Get all filenames with mtimes
  ;; 2. For every file, generate all outbound links (above the --Backlinks-- line)
  ;; 3. Add those links to our target -> [...inbound_links] hash
  ;; 4. Go through hash and update the --Backlinks-- section for every file
  ;; 5. Reset all mtimes?

  ;; TODO
  ;; Add the save hook, play around, ensure functionality
  ;; Clean up and integrate into zettel-mode

  ;;; Code:
  ;; TODO this should take directory as a parameter
  (defun gw/all-notes-w-mtimes ()
    "Get a list of (filename . mtime) pairs.
  Output format: ((<abs-note-path> . <mtime timestamp str>) (<abs-note-path> . <mtime timestamp str>))"
    (mapcar
     (lambda (file_and_attr)
       (cons
        (car file_and_attr)
        (file-attribute-modification-time (cdr file_and_attr))))
     (directory-files-and-attributes gw/zettel-dir-truename t "^[A-Z]")))

  (defun gw/generated-end-idx ()
    "Get the position of the end of the generated heading.
  Meant to be called inside with-temp-buffer. Returns point-max if
  there's no generated heading.
  Returns: int"
    (point-min)
    (save-excursion
      (let ((case-fold-search nil))
        (or (search-forward "# Backlinks" nil t)
            (point-max)))))


  (defun gw/outbound-links (filepath)
    "Get all unique outbound (above `# Backlinks`) links for a note.
  Returns: (\"Memex\" \"Activities\")"
    (with-temp-buffer
       (insert-file-contents filepath)
      (let ((generated-idx (gw/generated-end-idx))
            (res))
        (while (re-search-forward "\\[\\[\\(.+\\)\\]\\]" generated-idx t)
          ;; add-to-list dedups
  ...
  (defun gw/gen-backlinks ()
    ...
    (let ((res (make-hash-table :test #'equal))
          (notes (gw/all-notes-w-mtimes)))
      (mapc
       (lambda (note)
         (mapc
          (lambda (link-target)
            (puthash link-target (append (gethash link-target res ()) (list (car note))) res))
             ...
             (let ((linkers (gethash (file-name-base (car note)) res)))
               (if linkers
                   (progn
                     ...
                     (setq modified t))))
             (when modified
               ;; Doesn't run save-hooks, which will include gw/gen-backlinks
