...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Rebalance and switch windows after adding / removing splits
...
#+BEGIN_SRC emacs-lisp
  ...
  Removes the current buffer's file from the candidate list."
    (interactive)
    (ivy-read "Select note: "
              (seq-filter
               (lambda (note)
                 (not (string= note (file-name-base (buffer-file-name)))))
               (gw/all-notes))
              :action (lambda (note)
  ...
  (define-minor-mode gw/zettel-mode
    ...
    :keymap (let ((map (make-sparse-keymap)))
              ...
              ;; Follow link if we're in one, otherwise, next buffer
              (define-key map (kbd "<right>") (lambda ()
                                                (interactive)
                                                (if (markdown-inside-link-p)
              ...
              ;; Passing `t' opens in the next window (if any) but changes focus by default
