...
#+BEGIN_SRC emacs-lisp
  (use-package company
    ...
    :config
    (setq company-idle-delay 0.3)           ; In seconds--default is 0.5
...
** Init windmove keybindings (Ctrl-arrow)
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  (use-package counsel
    ...
    ("C-s" . counsel-grep-or-swiper) ; Replace isearch-forward with swiper
...
** C-x f: Jump to file below pwd instead of set-fill-column
